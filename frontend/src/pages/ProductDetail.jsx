...
function SizeGuide({ productType, gender }) {
  ...
  let sizeData = [], title = ''
  if (isPants) {
    sizeData = gender === 'Nam' ? pantsSizesMale : gender === 'Nữ' ? pantsSizesFemale : pantsSizesMale
    title = gender === 'Nam' ? 'Bảng size quần Nam' : gender === 'Nữ' ? 'Bảng size quần Nữ' : 'Bảng size quần'
  } else {
    sizeData = gender === 'Nam' ? shirtSizesMale : gender === 'Nữ' ? shirtSizesFemale : shirtSizesMale
    title = gender === 'Nam' ? 'Bảng size áo Nam' : gender === 'Nữ' ? 'Bảng size áo Nữ' : 'Bảng size'
  }

  return (
    <div className="mt-4">
      <button onClick={() => setIsOpen(!isOpen)} className="flex items-center gap-2 text-blue-600 hover:text-blue-800 font-medium text-sm transition-colors">
  ...
  return (
    <div className="flex items-center gap-2">
      ...
      <div className="flex gap-1">
        {timeLeft.days > 0 && <div className="bg-red-600 text-white px-2 py-1 rounded font-bold text-sm">{timeLeft.days} ngày</div>}
        <div className="bg-red-600 text-white px-2 py-1 rounded font-bold text-sm">{String(timeLeft.hours).padStart(2, '0')} giờ</div>
        <div className="bg-red-600 text-white px-2 py-1 rounded font-bold text-sm">{String(timeLeft.minutes).padStart(2, '0')} phút</div>
        <div className="bg-red-600 text-white px-2 py-1 rounded font-bold text-sm animate-pulse">{String(timeLeft.seconds).padStart(2, '0')} giây</div>
      </div>
    </div>
  )
}
...
export default function ProductDetail() {
  ...
  const trackProductView = async () => {
    try { await api.post('/api/recommendations/track-view', { product_id: parseInt(id) }) } catch (err) {}
  }
    ...
    <div className="max-w-7xl mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-12">
        <div className="relative">
          <img src={getImageUrl(product.hinh_anh) || 'https://via.placeholder.com/600'} alt={product.ten_san_pham} className="w-full rounded-2xl shadow-2xl" />
          {product.trang_thai === 'Het_hang' && <div className="absolute top-4 right-4 bg-red-500 text-white px-4 py-2 rounded-lg font-bold shadow-lg">Hết hàng</div>}
          {product.trang_thai === 'Con_hang' && <div className="absolute top-4 right-4 bg-green-500 text-white px-4 py-2 rounded-lg font-bold shadow-lg">Còn hàng</div>}
        </div>

        <div className="space-y-6">
          <div>
            <div className="flex items-center gap-2 mb-3">
              <span className="bg-teal-100 text-teal-800 px-3 py-1 rounded-full text-sm font-semibold">{product.loai}</span>
            ...
            {product.promotion ? (
              <div className="space-y-3 mb-6">
                ...
                <p className="text-4xl text-red-600 font-bold">{product.promotion.promotional_price?.toLocaleString('vi-VN')}</p>
                <p className="text-xl text-gray-500 line-through">Giá gốc: {product.gia_ban?.toLocaleString('vi-VN')}</p>
                <p className="text-lg text-green-600 font-semibold"> Tiết kiệm {(product.gia_ban - product.promotion.promotional_price)?.toLocaleString('vi-VN')}</p>
              </div>
            ) : (
              <p className="text-4xl text-blue-600 font-bold mb-6">{product.gia_ban?.toLocaleString('vi-VN')}</p>
            )}
          </div>

          <div className="bg-gray-50 rounded-xl p-6 space-y-3">
            {product.chat_lieu && <div className="flex items-center"><span className="font-semibold text-gray-700 w-32">Chất liệu:</span><span className="text-gray-600">{product.chat_lieu}</span></div>}
            {product.size && <div className="flex items-center"><span className="font-semibold text-gray-700 w-32">Size có sẵn:</span><span className="text-gray-600">{product.size}</span></div>}
          </div>
          ...
          {availableSizes.length > 0 && (
            <div>
              <label className="block font-bold text-lg mb-3 text-gray-800">Chọn size: <span className="text-red-500">*</span></label>
              <div className="flex flex-wrap gap-3">
                {availableSizes.map((size) => (
                  <button key={size} onClick={() => setSelectedSize(size)} className={`px-6 py-3 rounded-lg font-semibold transition-all ${selectedSize === size ? 'bg-blue-600 text-white shadow-lg scale-105' : 'bg-white border-2 border-gray-300 text-gray-700 hover:border-blue-500'}`}>{size}</button>
                ))}
              </div>
          ...
          <div>
            ...
            <div className="flex items-center gap-4">
              <button onClick={() => setQuantity(Math.max(1, quantity - 1))} className="w-12 h-12 bg-gray-200 rounded-lg font-bold text-xl hover:bg-gray-300 transition">-</button>
              <input type="number" min="1" value={quantity} onChange={(e) => setQuantity(Math.max(1, parseInt(e.target.value) || 1))} className="w-20 h-12 text-center text-xl font-bold border-2 border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-transparent" />
              <button onClick={() => setQuantity(quantity + 1)} className="w-12 h-12 bg-gray-200 rounded-lg font-bold text-xl hover:bg-gray-300 transition">+</button>
            </div>
          </div>

          {message.text && <div className={`px-6 py-4 rounded-xl font-semibold ${message.type === 'success' ? 'bg-green-100 border-2 border-green-400 text-green-700' : 'bg-red-100 border-2 border-red-400 text-red-700'}`}>{message.text}</div>}

          <div className="flex gap-4">
            <button onClick={handleAddToCart} disabled={product.trang_thai === 'Het_hang'} className={`flex-1 px-8 py-4 rounded-xl font-bold text-lg transition-all shadow-lg ${product.trang_thai === 'Het_hang' ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-white border-2 border-blue-600 text-blue-600 hover:bg-blue-50'}`}> Thêm vào giỏ</button>
            <button onClick={handleBuyNow} disabled={product.trang_thai === 'Het_hang'} className={`flex-1 px-8 py-4 rounded-xl font-bold text-lg transition-all shadow-lg ${product.trang_thai === 'Het_hang' ? 'bg-gray-300 text-gray-500 cursor-not-allowed' : 'bg-gradient-to-r from-blue-500 to-blue-600 text-white hover:from-blue-600 hover:to-blue-700'}`}> Mua ngay</button>
          </div>
        </div>
      </div>
